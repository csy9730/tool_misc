...
群延迟
$$
\tau _g (\omega)=-\frac{d\phi(\omega)}{d\omega}
$$

相位延迟 phase delay
$$
\tau _p (\omega)=-\frac{\phi(\omega)}{\omega}
$$
#### 全通系统
...
- 数字滤波器和模拟滤波器
- 滤波器类型：低通，高通，带通，带阻
- 通带（中心）频率，阻带频率，通带带宽，响应时间

#### 滤波器设计
滤波器的核心就是过渡带，通带和阻带不是核心。
...
一般来说：过渡带的幅值衰减越大，相位延迟越大。
我们期望，幅值衰减越大越好，相位延迟越小越好。

越陡的过渡带，对应越高阶的滤波器。想要幅值衰减大，就要提高滤波器阶数，提高阶数，必然引入相位延迟。
...
2
$$
H(z)= \frac{1}{N}\sum_{n=0}^{N-1}{z^{-n}}=\frac{1}{N} \frac{1-z^{-N}}{1-z^{-1}}
$$
3
$$
y(n)=\frac{1}{N}[x(n)+x(n-1)+...x(n-N+1)]
$$
...
$$
H_1(z)=b\frac{1-z^{-N}}{1-\rho z^{-N}}, b=\frac{1+\rho}{2}
$$

2
$$
H_2(z)=b\frac{1+z^{-N}}{1-\rho z^{-N}},b=\frac{1-\rho}{2}
$$
#### 互补滤波器
$$
H_1(z)+H_2(z)=1
