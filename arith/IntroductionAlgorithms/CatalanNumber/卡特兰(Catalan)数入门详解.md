...
在一个w×h的网格上，你最开始在(0,0)(0,0)上，你每个单位时间可以向上走一格，或者向右走一格，在任意一个时刻，你往右走的次数都不能少于往上走的次数，问走到(n,m),0≤n(n,m),0≤n有多少种不同的合法路径。

**合法路径个数为Cn2n−Cn−12nC2nn−C2nn−1**

直接求不好，考虑求有多少种不合法路径
路径总数为在2n次移动中选n次向上移动，即Cn2nC2nn
...
我们把(a,a+1)(a,a+1)之后的路径全部按照y=x+1这条线对称过去
